        ...
        try:
            result = subprocess.run(
                [
                    "powershell",
                    "-Command",
                    '$mb = Get-WmiObject -Class Win32_BaseBoard; $bios = Get-WmiObject -Class Win32_BIOS; $system = Get-WmiObject -Class Win32_ComputerSystem; $memArray = Get-WmiObject -Class Win32_PhysicalMemoryArray; $memModules = Get-WmiObject -Class Win32_PhysicalMemory; Write-Host "Motherboard: $($mb.Product)"; Write-Host "Manufacturer: $($mb.Manufacturer)"; Write-Host "Version: $($mb.Version)"; Write-Host "Chipset: AMD B650"; Write-Host "BIOS Version: $($bios.SMBIOSBIOSVersion)"; Write-Host "BIOS Manufacturer: $($bios.Manufacturer)"; Write-Host "BIOS Date: $([DateTime]::ParseExact($bios.ReleaseDate.Substring(0,8), \'yyyyMMdd\', $null).ToString(\'MM/dd/yyyy\'))"; Write-Host "System Model: $($system.Model)"; Write-Host "Memory Slots: $($memArray.MemoryDevices)"; Write-Host "Max Memory Capacity: $([math]::Round($memArray.MaxCapacity/1024/1024)) GB"; Write-Host "Memory Slots Used: $($memModules.Count)"',
                ],
                capture_output=True,
                text=True,
                timeout=10,
